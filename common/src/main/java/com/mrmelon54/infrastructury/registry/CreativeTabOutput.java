...
public interface CreativeTabOutput extends CreativeModeTab.Output {
    ...
    default void acceptAllAfter(ItemStack after, Collection<ItemStack> stacks, CreativeModeTab.TabVisibility visibility) {
        stacks.forEach((stack) -> {
            this.acceptAfter(after, stack, visibility);
        });
    }
    ...
    default void acceptAllBefore(ItemStack before, Collection<ItemStack> stacks, CreativeModeTab.TabVisibility visibility) {
        stacks.forEach((stack) -> {
            this.acceptBefore(before, stack, visibility);
        });
    }
    ...
    static remapped.architectury.registry.CreativeTabOutput convert(CreativeTabOutput tab) {
        return new remapped.architectury.registry.CreativeTabOutput() {
            @Override
            public void acceptAfter(ItemStack itemStack, ItemStack itemStack1, CreativeModeTab.TabVisibility tabVisibility) {
                tab.acceptAfter(itemStack, itemStack1, tabVisibility);
            }

            @Override
            public void acceptBefore(ItemStack itemStack, ItemStack itemStack1, CreativeModeTab.TabVisibility tabVisibility) {
                tab.acceptBefore(itemStack, itemStack1, tabVisibility);
            }
        };
    }

    static CreativeTabOutput convert(remapped.architectury.registry.CreativeTabOutput tab) {
